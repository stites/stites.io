...
Here's a sneak peak at what I'll be presenting on. I should probably mention that the idea of presenting on this subject was David Hall's idea, and the remainder of this post was from him in the form of an email which I've edited.

## Pseudo-classical Selectors in CSS

Pseudo-classical selectors (which I'll refer to as PCSs in this post) are pretty common and you've probably seen/used them already. I've used `:active` and `:hover` before - back when I wasn't really into CSS at all. I assume that you've probably come across these guy as well, if not others. Essentiall, pseudo classes are appended to your CSS selectors in your stylesheet. All PCSs are supported in IE9+, so that's a chunck of IEs that you don't have to worry about. They'll look something like this:
...
A cool way to think about them is that they're kind of like event-handlers for your CSS. Although, in realisty, PCSs are a mix of events and selections which you can narrow down. The

#### Legacy
These are the pseudo class selectors you encouter the most:
...
>     :visited
> > How to style an anchor whose remote link you have already visited. Most useful when adding links that are straight text, to keep it from turning purple after being clicked. (IE a:visited { text-decoration:none, color: #000 })

>     :focus
> > change styles when a field has focus - IE when you click inside an input form field, it has focus. Change the color of the border, add an inset box shadow, etc, to let the user know they have the field selected

Speaking of:

>     input[type="text"]
> > instead of added a separate class to every field in your form, use input[type=""] to target only specific input types in your form. (IE text, textarea, submit)

###### Narrow down your selections:

>      :nth-child()
> > Selects only the nth child of a given selector within the parent element. The most common types to put in the parens are even and odd, but you can also use some simple built in math to select every third, or whatnot. I wont explain how it works directly, but here is a good atricle on how it works: http://css-tricks.com/how-nth-child-works/
> > I use this all the time to give styled variations in when elements are being repeated over and over. For instance, if I have an UL with 20 LI's in it listing some generic information from my database. I can use
...
>     :first-line
> > Same as frist-letter, but affects the first line of text different from all proceeding
...
Add this to your main styles. Now any element you give a class of unselectable to cannot be selected with a mouse. SUPER useful when your nav contains text, and you dont want users to be selected the nav text when a click on a nav item turns into a double click selection.
...
> > Add these at the bottom of your styles page to change how the CSS is rendered as your page gets narrower. This is the best way to handle responsive design. You will need mutiple of these to handle all the different situations, going from the largest to smallest in PX. Check out: http://css-tricks.com/css-media-queries/